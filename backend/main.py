...
from routers import projects, outlines, chapters, entities, rag, ai, characters, fanqie, preferences, jobs, timeline, graph, pipeline, auth, admin, membership
...
app.include_router(jobs.router, prefix="/api/jobs", tags=["Writing Jobs"])
app.include_router(timeline.router, prefix="/api/timeline", tags=["Event Timeline"])
app.include_router(graph.router, prefix="/api/graph", tags=["LangGraph Workflows"])
app.include_router(pipeline.router, prefix="/api/pipeline", tags=["Novel Pipeline"])
