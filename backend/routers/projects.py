...
from services.membership_service import require_auth_user
...
class ProjectStatus(BaseModel):
    ...
    total_words: int = 0
    chapter_word_count: Optional[int] = None  # 单章目标字数
...
async def list_projects(authorization: str | None = Header(None)):
    ...
    projects = projects_manager.list_projects(user.id)
    current = projects_manager.get_current_project(user.id)
    return {
        "projects": projects,
        "current": current
    }
...
async def get_status(root: Path = Depends(get_project_root)):
    ...
    if state:
        ...
        if info:
            ...
            status.chapter_word_count = info.get("chapter_word_count") or state.get("chapter_word_count")
            ...
            status.chapter_word_count = state.get("chapter_word_count")
...
async def update_info(info: ProjectInfoUpdate, root: Path = Depends(get_project_root)):
    ...
    try:
        ...
        if info.chapter_word_count is not None:
            if info.chapter_word_count < 2000 or info.chapter_word_count > 5000 or info.chapter_word_count % 500 != 0:
                raise HTTPException(status_code=400, detail="单章字数必须在 2000-5000 之间，且以 500 递增")
            state["project_info"]["chapter_word_count"] = info.chapter_word_count
            state["chapter_word_count"] = info.chapter_word_count
...
@router.post("/switch")
async def switch_project(request: ProjectSwitchRequest, authorization: str | None = Header(None)):
    """切换当前活动项目"""
    ...
    user = require_auth_user(authorization)
    switch_result = projects_manager.switch_project(str(path), user.id)
    if "error" in switch_result:
        raise HTTPException(status_code=404, detail=switch_result["error"])
...
@router.post("/create")
async def create_project_api(request: ProjectCreateRequest, authorization: str | None = Header(None)):
    """创建新项目"""
    user = require_auth_user(authorization)
    result = projects_manager.create_project(request.name, request.path, request.genre, request.substyle, user.id)
    if "error" in result:
...
@router.post("/import")
async def import_project_api(request: ImportRequest, authorization: str | None = Header(None)):
    """导入现有项目"""
    user = require_auth_user(authorization)
    result = projects_manager.import_project(request.path, user.id)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result
...
async def delete_project_api(project_id: str, delete_files: bool = False, authorization: str | None = Header(None)):
    """删除项目记录"""
    user = require_auth_user(authorization)
    result = projects_manager.delete_project(project_id, delete_files, user.id)
    if "error" in result:
