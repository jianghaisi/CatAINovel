...
from services.user_preferences import build_preference_prompt_context, load_user_preferences
...
class SkillExecutor:
    ...
    def _normalize_chapter_word_count(self, word_count: Optional[int] = None) -> int:
        try:
            target = int(word_count or 0)
        except Exception:
            target = 0
        if target <= 0:
            state = self._load_state() or {}
            project_info = state.get("project_info", {}) if isinstance(state, dict) else {}
            try:
                target = int(project_info.get("chapter_word_count") or state.get("chapter_word_count") or 3500)
            except Exception:
                target = 3500
        return max(2000, min(target, 5000))

    def _chapter_word_count_bounds(self, word_count: Optional[int] = None) -> Dict[str, int]:
        target = self._normalize_chapter_word_count(word_count)
        return {
            "word_count": target,
            "word_count_min": max(1200, target - 300),
            "word_count_max": target + 600,
            "word_count_hard_cap": target + 900,
        }

    def _apply_dynamic_word_count_constraints(self, text: str, word_count: Optional[int] = None) -> str:
        """Keep old project prompt snapshots aligned with the current chapter target."""
        value = self._safe_text(text)
        bounds = self._chapter_word_count_bounds(word_count)
        soft_range = f"{bounds['word_count']}-{bounds['word_count_max']}"
        hard_cap = str(bounds["word_count_hard_cap"])
        replacements = {
            "3200-3800字": f"{soft_range}字",
            "3200-3800 字": f"{soft_range} 字",
            "3200-3800瀛?": f"{soft_range}字",
            "3200-3800 瀛?": f"{soft_range} 字",
            "严禁超过 4000 字": f"严禁超过 {hard_cap} 字",
            "严禁超过4000字": f"严禁超过 {hard_cap} 字",
            "涓ョ瓒呰繃 4000 瀛?": f"严禁超过 {hard_cap} 字",
            "涓ョ瓒呰繃 4000 瀛楋": f"严禁超过 {hard_cap} 字",
        }
        for old, new in replacements.items():
            value = value.replace(old, new)
        value = re.sub(r"严禁超过\s*4000\s*字", f"严禁超过 {hard_cap} 字", value)
        value = re.sub(r"控制在\s*\d{3,5}\s*-\s*\d{3,5}\s*字", f"控制在 {soft_range} 字", value)
        return value
...
""",
    ...
    async def execute_init_stream(
        ...
        skip_legacy_outline: bool = False,
    ):
    ...
    async def _ai_fill_init_content_stream(
        self,
        title,
        genre,
        substyle,
...
要求：人物鲜明、金手指有成长空间、符合爽文节奏。使用 Markdown 格式。"""
        ...
        if skip_legacy_outline:
            yield {"type": "step", "step": 10, "name": "旧版总纲生成", "status": "skipped"}
            return
   ...
   - **卷末高潮**：..."""
    ...
    async def execute_init(
        ...
        async for update_str in self.execute_init_stream(
            ...
            skip_legacy_outline=skip_legacy_outline,
        ):
...
请根据要求润色大纲（直接输出正文）："""
    ...
    def _current_protagonist_realm(self) -> str:
        state = self._load_state() or {}
        pstate = state.get("protagonist_state", {}) if isinstance(state.get("protagonist_state"), dict) else {}
        realm = self._safe_text(pstate.get("realm", "")).strip()
        if realm:
            return realm

        protagonist_name = self._resolve_protagonist_name()
        profile = self._find_character_file_by_name(protagonist_name) if protagonist_name else None
        if profile and profile.exists():
            text = self._read_file(profile)
            match = re.search(r"(?:当前境界|境界)\*\*?[：:]\s*([^\n\r]+)", text)
            if match:
                return self._safe_text(match.group(1)).strip()
        return ""

    def _validate_realm_before_write(self) -> List[str]:
        preference = load_user_preferences(self.project_root)
        realm_names = [item.name for item in preference.realms if item.name]
        current_realm = self._current_protagonist_realm()
        if not realm_names or not current_realm or current_realm in {"未知", "未填写", "待补充"}:
            return []
        if current_realm not in realm_names:
            return [f"当前主角境界「{current_realm}」不在自定义境界表中：{'、'.join(realm_names)}"]
        return []

    def _template_to_regex(self, template: str) -> re.Pattern:
        escaped = re.escape(template or "")
        pattern = re.sub(r"\\\{[^{}]+\\\}", r".+?", escaped)
        return re.compile(rf"^{pattern}$")

    def _find_system_panel_lines(self, content: str) -> List[str]:
        panel_keywords = ("系统", "宿主", "等级", "属性", "技能", "任务", "面板")
        lines = []
        for line in self._safe_text(content).splitlines():
            stripped = line.strip()
            if not stripped:
                continue
            if any(keyword in stripped for keyword in panel_keywords) and (
                stripped.startswith("[") or stripped.startswith("【") or stripped.startswith("|")
            ):
                lines.append(stripped)
        return lines

    def _validate_system_ui_template(self, content: str) -> List[str]:
        preference = load_user_preferences(self.project_root)
        template = self._safe_text(preference.system_ui_template).strip()
        if not template:
            return []
        panel_lines = self._find_system_panel_lines(content)
        if not panel_lines:
            return []
        matcher = self._template_to_regex(template)
        invalid = [line for line in panel_lines if not matcher.match(line)]
        if not invalid:
            return []
        return [
            "系统面板未 100% 匹配用户模版："
            f"模版={template}；不匹配示例={' / '.join(invalid[:3])}"
        ]

    async def _repair_system_ui_template(self, content: str, chapter: int) -> str:
        preference = load_user_preferences(self.project_root)
        template = self._safe_text(preference.system_ui_template).strip()
        if not template or not self.ai_service:
            return content
        issues = self._validate_system_ui_template(content)
        if not issues:
            return content

        prompt = f"""请只修正第{chapter}章正文中的系统面板/系统弹窗格式，不要改动其它正文。

【必须使用的系统 UI 模版】
{template}

【修正规则】
1. 所有系统面板行必须严格匹配该模版；只允许替换 {{name}}、{{level}} 等花括号变量的值。
2. 固定文字、括号、竖线、空格、冒号必须 100% 保持一致。
3. 如果某个系统面板无法可靠套入模版，请删除该面板行，不要新造 UI 样式。
4. 直接输出修正后的完整正文，不要解释。

【待修正文】
{self._truncate_text(content, 12000, keep_tail=True)}
"""
        repaired = await self.ai_service.chat(
            [{"role": "user", "content": prompt}],
            temperature=0.1,
            max_tokens=12000,
        )
        return self._safe_text(repaired).strip() or content

    async def _continue_chapter_if_incomplete(
        self,
        chapter: int,
        chapter_outline: str,
        content: str,
        ...
        full_content = self._safe_text(content)
        bounds = self._chapter_word_count_bounds(word_count)
        target_min = max(1200, bounds["word_count"] - 500)
        for _ in range(max_rounds):
            if len(full_content.strip()) >= target_min and not self._has_abrupt_tail(full_content):
                break

            prompt = f"""你正在续写第{chapter}章正文。前文可能因为模型或网络中断而没有写完，请从最后一句之后自然续写并收束本章。

【本章大纲】
{self._truncate_text(chapter_outline, 1200, keep_tail=False)}

【已生成正文末尾】
{self._truncate_text(full_content, 1800, keep_tail=True)}

【续写要求】
1. 只输出续写内容，不要复述前文。
2. 保持时间、地点、人物状态连续。
3. 如果已经接近完整，请用 2-6 段完成收束；不要把下一章核心高潮提前写完。
4. 系统面板若出现，必须严格使用用户定义的系统 UI 模版。
"""
            appendix = await self.ai_service.chat(
                [{"role": "user", "content": prompt}],
                temperature=0.45,
    ...
    async def execute_write_stream(self, chapter: int, word_count: int = 3500):
        ...
        yield make_event("step", name="境界与系统模版预校验", status="processing")
        realm_errors = self._validate_realm_before_write()
        if realm_errors:
            yield make_event("error", message="；".join(realm_errors))
            return
        yield make_event("step", name="境界与系统模版预校验", status="completed")
        ...
        if self.ai_service:
            try:
                ...
                hard_cap = self._chapter_word_count_bounds(word_count)["word_count_hard_cap"]
                if raw_len > hard_cap:
                    self._debug(f"[WRITE-LENGTH WARN] 第{chapter}章字数 {raw_len}，超过 4000 字目标，但不截断以保证章节完整性")

                full_content = await self._continue_chapter_if_incomplete(
                    chapter=chapter,
                ...
                yield make_event("step", name="系统面板模版校验", status="processing")
                full_content = await self._repair_system_ui_template(full_content, chapter)
                ui_errors = self._validate_system_ui_template(full_content)
                if ui_errors:
                    yield make_event("error", message="；".join(ui_errors))
                    return
                if full_content != streamed_content:
                    yield make_event("content", chunk="", full=full_content, replace=True)
                yield make_event("step", name="系统面板模版校验", status="completed")
...
"""
    ...
    def _build_chapter_hard_constraints_prompt(
        ...
        *,
        ...
        if template:
            formatted = self._format_prompt_text(
                template,
                core_constraints=safe_constraints,
                worldview=safe_worldview,
                protagonist_name=safe_name,
                protagonist_desc=safe_desc,
                word_count=str(bounds["word_count"]),
                word_count_min=str(bounds["word_count_min"]),
                word_count_max=str(bounds["word_count_max"]),
                word_count_hard_cap=str(bounds["word_count_hard_cap"]),
            )
            return self._apply_dynamic_word_count_constraints(formatted, bounds["word_count"])
...
"""
    ...
    async def _generate_chapter_content_stream(
        ...
        preference_prompt_context = build_preference_prompt_context(self.project_root)
        if preference_prompt_context:
...
"""
        ...
        word_bounds = self._chapter_word_count_bounds(word_count)
        if writing_user_template:
            context = self._format_prompt_text(
                ...
                word_count=str(word_bounds["word_count"]),
                word_count_min=str(word_bounds["word_count_min"]),
                word_count_max=str(word_bounds["word_count_max"]),
                word_count_hard_cap=str(word_bounds["word_count_hard_cap"]),
            )
        else:
            # Fallback：原始硬编码
...
10. **在适当位置结尾**：再次检查【剧情边界红线】！本章结尾必须在下一章内容**之前**停止，留下悬念！"""
        ...
        context = self._apply_dynamic_word_count_constraints(context, word_bounds["word_count"])
