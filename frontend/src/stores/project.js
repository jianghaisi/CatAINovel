...
import api, { projectsApi, chaptersApi, setProjectRootHeader } from '../api'

export const useProjectStore = defineStore('project', {
    state: () => {
        const root = sessionStorage.getItem('webnovel_project_root') || ''
        if (root) setProjectRootHeader(root)
        return {
            initialized: false,
            projectRoot: root,
            currentChapter: 0,
        ...
        async fetchStatus() {
            ...
            try {
                ...
                if (data.project_root && data.project_root !== this.projectRoot) {
                    ...
                    setProjectRootHeader(this.projectRoot)
                }
                ...
                this.chapterWordCount = data.chapter_word_count || 3500
